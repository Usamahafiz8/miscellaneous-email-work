...
export function describeImapError(err: unknown, config?: Pick<IMAPConfig, "host" | "port">): string {
  ...
  if (/ETIMEDOUT|timed out/i.test(raw)) {
    // Must describe the port actually used, not a port we assume was meant.
    // This previously hardcoded advice about 143 and so told someone who had
    // typed 140 that "port 143 will hang like this" — naming a port they never
    // entered, which reads like the app is confused about its own settings.
    // (143 itself never reaches here now; explainWrongPort rejects it upfront.)
    const port = config?.port;
    const portNote = port && port !== 993
      ? ` Nothing is listening on port ${port} — IMAP over SSL is almost always port 993.`
      : " A firewall may be blocking IMAP, or the host may be wrong.";
    return `The mail server didn't respond${where}.${portNote}`;
  }
